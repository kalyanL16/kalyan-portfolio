...
export default function Header() {
  ...
  // Keep your labels the same, but we'll force Contact -> /contact below
  const nav = useMemo(
    () => [
      { label: "Projects", href: "/projects" },
      { label: "Education", href: "/education" },
      { label: "Achievements", href: "/achievements" },
      { label: "Contact", href: "/contact" }, // ensure this is the About page route
    ],
    []
  );
      ...
      <div className="mx-auto flex w-[min(1200px,92%)] items-center justify-between py-3">
        ...
        <nav className="flex items-center gap-2">
          {nav
            ...
            .map((item) => {
              // Force Contact to always go to /contact (About page)
              const targetHref = item.label === "Contact" ? "/contact" : item.href;
